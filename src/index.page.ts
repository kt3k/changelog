...
// with a daily summary. The strip is interactive — hovering or tapping a cell
// with activity moves the accent ring to it and swaps the summary below to
// that day's; cells with no activity (size:N / no post) are inert, leaving the
// ring and summary on the last active day. Each repo's 40 days of title +
// excerpt are embedded as JSON for the client script. On touch a tap acts like
// hover and is kept from following the card link (preventDefault +
// stopPropagation).
