// EXPERIMENT (index-ab): combines approach A (explicit scope labels) and
// approach B (anchor the excerpt to its own cell in the 40-day strip). The
// strip is labelled "Last 40 days" and the latest-day summary gets its own
// "Latest · <date>" label, while the cell that summary describes is ringed —
// so the strip reads as a 40-day overview and the excerpt as one single day.
export const layout = "layouts/base.vto";
export const url = "/index-ab.html";
...
export default function (data: Lume.Data): string {
  ...
  const cards = watched.map((repo) => {
    ...
    // Scope label (A): name the timespan so the strip can't be read as "today".
    const stat =
    ...
    if (latest && latestDate) {
      // Scope label (A) on the prose, dated + accent-tinted to visually pair
      // with the ringed cell above.
      latestBlock = `
